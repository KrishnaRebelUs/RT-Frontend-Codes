...
const YearlyTrend = () => {
  ...
  const TypographyStyled = styled(Typography)(({ theme, index }) => ({
    color:
      index % 3 === 0 ? "#2edd95" : index % 3 === 1 ? "#f89c53" : "#245aa0",
  }));
    ...
    const fetchYearlyData = async () => {
      ...
      try {
        const response = await axios.get(
          ...
          `${BASE_URL}/shortage/getYoyFindingAll?vendorId=${selectedVendorId}&pageName=Shortage`,
          {
            headers: {
  ...
  const getTopThreeYears = () => {
    ...
    for (let i = 0; i < sortedYears.length; i++) {
      if (
        sequentialYears.length === 0 ||
        sortedYears[i] === sequentialYears[sequentialYears.length - 1] - 1
      ) {
  ...
  return (
    ...
    >
      ...
      <Typography variant="h3" sx={{ color: "#000000" }}>
        ...
        }).format(Math.abs(parseFloat(yearlyData[getTopThreeYears()[0]] || 0)))}
      </Typography>
              ...
              <Typography variant="h6">
                <b>
                  ...
                  {new Intl.NumberFormat(undefined, {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  }).format(Math.abs(parseFloat(yearlyData[year] || 0)))}
                </b>
              </Typography>
            </Grid>
          ))}
