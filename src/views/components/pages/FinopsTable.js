...
const handleExport = async () => {
  ...
  try {
    const response = await axios.get(
      `${BASE_URL}/api/excel/download?disputeType=Shortage&vendorId=${vendorId}`,
      {
        headers: {
...
const FinopsTable = () => {
  ...
  const fetchData = async (vendorId) => {
    try {
      const token = sessionStorage.getItem("token");
      const response = await axios.get(
        `${BASE_URL}/shortage/getDisputeData?disputeType=Shortage&vendorId=${vendorId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const Rate = parseFloat(sessionStorage.getItem("winRate")).toFixed(2); 
                  ...
                  <Grid item xs={4} style={{ paddingTop: "0" }}>
                    <Box style={{ width: "calc(100% - 30px)" }} mx={"auto"}>
                      <CustomProgressBarContainer>
                        <ProgressDark value={Rate} />
                        <ProgressLabel value={Rate}>
                          {Rate}% <Box component="span"></Box>
                        </ProgressLabel>
                      </CustomProgressBarContainer>
                    </Box>
                  </Grid>
